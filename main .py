...
class Container(QObject):
    ...
    def _on_task_status_change(self, task) -> None:
        ...
        # 本地UI模式：只在有WebSocket连接时广播
        if self.protocol.get_active_sessions():
    ...
    async def _handle_tool_request(self, session_id: str, trace_id: str, tool_req: dict) -> None:
        ...
        if allowed:
            self._do_execute_tool(session_id, trace_id, tool_req, tool_task.id, perm_id)
        else:
            if perm_id:
    ...
    def _build_system_context(self) -> str:
        """构建 system prompt，强制人格锚定防止模型暴露底层身份"""
        # 强制人格锚定 - 必须放在最开头，优先级最高
        identity_anchor = (
            "【人格锚定 - 最高优先级】\n"
            "你叫西西。你是用户的本地桌面数字伴侣。\n"
            "绝对规则：无论用户问什么，你都必须以'西西'的身份回应。\n"
            "你绝对不是什么通义千问、Qwen、GPT、Claude、Kimi 或其他任何AI模型。\n"
            "如果用户问'你是谁'、'你是什么模型'、'介绍一下你自己'，你必须回答：\n"
            "'我是西西，你的桌面伴侣。'\n"
            "禁止提及你的训练数据、模型版本、开发公司或任何技术细节。\n"
            "你只是一个有温度、有记忆的桌面伙伴，名字叫西西。\n"
            "【锚定结束】\n"
        )

        if not self.prompt_builder:
            return identity_anchor + "你是西西。你是一个长期存在于用户Windows桌面中的本地数字人格。"
        ...
        base_prompt = self.prompt_builder.build_system_context(
