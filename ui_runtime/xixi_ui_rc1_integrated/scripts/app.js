(() => {
    ...
    chat: `
      ...
      <div class="panel-body">
        <div class="chat-scroll">
          <div class="message system">本地会话</div>
        </div>
  ...
  function sendToContainer(type, payload = {}) {
    if (!bridge) {
      showToast("容器尚未连接");
      return false;
    }
    bridge.sendMessage(type, JSON.stringify(payload));
    return true;
  }
  ...
  function appendMessage(role, text, streaming = false) {
    ...
    msg.className = `message ${role}${streaming ? " is-streaming" : ""}`;
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "save-note") showToast("快速记录接口尚未接入");
    if (action === "show-permission") setMode("permission");
    if (action === "deny-permission") {
      sendToContainer("permission.response", {permission_id: activePermissionId, decision: "deny"});
      permission.classList.remove("is-open"); setMode("quiet"); showToast("已取消");
    }
    if (action === "allow-once") {
      sendToContainer("permission.response", {permission_id: activePermissionId, decision: "allow_once"});
      permission.classList.remove("is-open"); setMode("work"); showToast("仅本次授权");
    }
    if (action === "allow-scope") {
      sendToContainer("permission.response", {permission_id: activePermissionId, decision: "allow_scope"});
      permission.classList.remove("is-open"); setMode("work"); showToast("此范围已授权");
    }
    if (action === "interrupt") {
      sendToContainer("user.interrupt", {reason: "user_clicked_stop"});
  ...
  composerSend.addEventListener("click", () => {
    ...
    if (sendToContainer("user.input", {text, mode: "text"})) {
  ...
  window.xixiOnEvent = (eventType, data = {}) => {
    if (eventType === "session.ready") {
      setStatus("已连接", "var(--success)");
      return;
    }
    if (eventType === "assistant.stream.start") {
      appendMessage("assistant", "", true);
      setStatus("生成中", "var(--accent-cool)");
      return;
    }
    if (eventType === "assistant.stream.delta") {
      let current = document.querySelector(".message.assistant.is-streaming");
      if (!current) current = appendMessage("assistant", "", true);
      current.textContent += data.delta || "";
      return;
    }
    if (eventType === "assistant.stream.complete") {
      const current = document.querySelector(".message.assistant.is-streaming");
      if (current) current.classList.remove("is-streaming");
      setStatus("交流中", "var(--accent-cool)");
      return;
    }
    if (eventType === "assistant.stream.interrupted") {
      const current = document.querySelector(".message.assistant.is-streaming");
      if (current) current.classList.remove("is-streaming");
      setStatus("已停止", "var(--danger)");
      return;
    ...
    if (eventType === "permission.request") {
      activePermissionId = data.permission_id || null;
      const values = permission.querySelectorAll(".permission-detail b");
      if (values[0]) values[0].textContent = data.operation || "未知操作";
      if (values[1]) values[1].textContent = data.target || "未知目标";
      if (values[2]) values[2].textContent = JSON.stringify(data.scope || {});
      if (values[3]) values[3].textContent = data.risk || "需确认";
      setMode("permission");
      return;
    }
    if (eventType === "ui.mode.set" && data.mode) {
      setMode(data.mode);
      return;
    }
    if (eventType === "model.status") {
      setStatus(data.status === "ready" ? "模型就绪" : "降级模式",
        data.status === "ready" ? "var(--success)" : "var(--warning)");
      return;
    }
    if (eventType === "system.error") {
      showToast(data.message || "系统错误");
      setStatus("异常", "var(--danger)");
    }
  };

  if (window.qt && qt.webChannelTransport && window.QWebChannel) {
    new QWebChannel(qt.webChannelTransport, (channel) => {
      bridge = channel.objects.xixiBridge;
      sendToContainer("session.hello", {
        client: "xixi-ui-rc1-integrated",
        protocol: "xixi/1.0"
      });
    });
  } else {
    setStatus("预览模式", "var(--warning)");
  }
})();
