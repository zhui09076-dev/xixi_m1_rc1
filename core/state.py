...
- 8个基础状态
- 启动模式
- 状态分层（state/emotion/relationship_state/attention/boot_mode）
- 保存后读取不丢字段
"""
...
class StateMachine:
    ...
    def from_dict(cls, d: Dict) -> "StateMachine":
        ...
        sm = cls(
            initial_state=XiXiState(state_val),
            boot_mode=BootMode(boot_mode)
        )

        if snapshot_data:
            sm.snapshot = StateSnapshot(**snapshot_data)
